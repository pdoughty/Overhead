...
ABSTRACT_DECLARATION( idClass, idEntity )
	...
	// PHIL BEGIN
	EVENT( EV_Interact,				idItem::Event_Interact )
...
============
*/
bool idEntity::InteractTouchingTriggers( void ) const {
   int            i, numClipModels, numEntities;
   idClipModel *   cm;
   idClipModel *   clipModels[ MAX_GENTITIES ];
   idEntity *      ent;
   ...
   for ( i = 0; i < numClipModels; i++ ) {
	  ...
	  /*if ( ent->spawnArgs.GetBool( "inv_item" ) && !ent->IsHidden() ) {
		  continue;
	  } else {
		  return;
	  }*/
      ...
      numEntities++;
      ent->ProcessEvent( &EV_Interact, this ); 

...
// PHIL BEGIN
	muzzleOrigin = playerViewOrigin;
	muzzleAxis = playerViewAxis;
	playerCursor.Draw(muzzleOrigin,muzzleAxis,"guis/mCrosshairs_fixed.tga");
// PHIL END
}
	...
	if ( gameLocal.isClient ) {
		...
		if ( projectileDict.GetBool( "net_instanthit" ) ) {
			...
			for( i = 0; i < num_projectiles; i++ ) {
				...
				dir = playerViewAxis[ 0 ] + playerViewAxis[ 2 ] * ( ang * idMath::Sin( spin ) ) - muzzleAxis[ 1 ] * ( ang * idMath::Cos( spin ) );
				dir.Normalize();

...
const idEventDef EV_Interact( "interact", "e" );
...
// PHIL BEGIN
	 if( Mountedflashlight.IsValid() ) {
      Mountedflashlight.GetEntity()->PostEventMS( &EV_Remove, 0 );
   }
   Mountedflashlight      = NULL;
// PHIL END
...
// PHIL BEGIN
		statsGui = uiManager->FindGui( "guis/stats.gui", true, false, true ); // finds the GUI associated with the stats menu
   invGui = uiManager->FindGui( "guis/inv.gui", true,false, true); //finds the GUI associated with the inventory menu
   mapGui = uiManager->FindGui( "guis/map.gui",true,false,true); //finds the GUI associated with the map menu
    startStatsGui = false; 
   startInvGui = false;
   startMapGui = false;


		statsUI = uiManager->FindGui( "guis/stats.gui", true, false, true ); // Set 'er up!
      statsUIopen = false;
      statsDebug = false;
// PHIL END
...
// PHIL BEGIN
	 if ( Mountedflashlight.IsValid() ) {
      Mountedflashlight.GetEntity()->ProcessEvent( &EV_Remove );
   }
// PHIL END
	if ( !gameLocal.isMultiplayer ) {
...
// PHIL BEGIN
	savefile->WriteUserInterface( statsUI, false );
   savefile->WriteBool( statsUIopen );
   savefile->WriteBool( statsDebug );
// PHIL END
...
// PHIL BEGIN
	Mountedflashlight.Save( savefile );
// PHIL END
...
// PHIL BEGIN
	savefile->ReadUserInterface( statsUI );
   savefile->ReadBool( statsUIopen ); 
   savefile->ReadBool( statsDebug );
// PHIL END
...
// PHIL BEGIN
	Mountedflashlight.Restore( savefile );
// PHIL END
...
void idPlayer::DrawHUD( idUserInterface *_hud ) {
	...
	if ( !GuiActive() ) {
		if ( cursor && weapon.GetEntity()->ShowCrosshair() && !statsUIopen ) { // PHIL
...
void idPlayer::NextWeapon( void ) {
	...
	if ( !weaponEnabled || spectating || hiddenWeapon || gameLocal.inCinematic || gameLocal.world->spawnArgs.GetBool( "no_Weapons" ) || health < 0 || statsUIopen) { // PHIL
		return;
	}
...
void idPlayer::SelectWeapon( int num, bool force ) {
	...
	if ( !weaponEnabled || spectating || gameLocal.inCinematic || health < 0 || statsUIopen ) { // PHIL
		return;
	}
...
void idPlayer::UpdateViewAngles( void ) {
	...
	if ( !noclip && ( gameLocal.inCinematic || privateCameraView || gameLocal.GetCamera() || influenceActive == INFLUENCE_LEVEL2 || objectiveSystemOpen || statsUIopen ) ) { // PHIL
...
/*
==============
idPlayer::ToggleStats
==============
*/
void idPlayer::ToggleStats(void){
   if(!startStatsGui)
   {
   statsGui->Activate( true, gameLocal.time );
   startStatsGui = true;
   }
   else{
   statsGui->Activate( false, gameLocal.time );
   startStatsGui = false;
   }
}
...
==============
*/
void idPlayer::ToggleInv(void){
   if(!startInvGui)
   {
   invGui->Activate( true, gameLocal.time );
   startInvGui = true;
   }
   else{
   invGui->Activate( false, gameLocal.time );
   startInvGui = false;
   }
}
...
==============
*/
void idPlayer::ToggleMap(void){
   if(!startMapGui)
   {
   mapGui->Activate( true, gameLocal.time );
   startMapGui = true;
   }
   else{
   mapGui->Activate( false, gameLocal.time );
   startMapGui = false;
   }
}
// PHIL END
...
// PHIL BEGIN
	case IMPULSE_19: {
         // when we're not in single player, IMPULSE_19 is used for showScores
         // otherwise it opens the pda
         if ( !gameLocal.isMultiplayer && !statsUIopen && !startInvGui ) {
            if ( objectiveSystemOpen ) {
               TogglePDA();
            } else if ( weapon_pda >= 0 ) {
               SelectWeapon( weapon_pda, true );
	...
	case IMPULSE_30: { 
         //common->Printf( "Impulse 30!\n" );
		 Interact(); // Interact Function
         break;
      }
// PHIL END
		case IMPULSE_20: {
		...
		case IMPULSE_40: {
	     ...
	     case IMPULSE_11: {
            if ( Mountedflashlight.IsValid() ) {
               ...
               const idDict *lightDef = gameLocal.FindEntityDefDict( "mounted_flashlight", false );
               if ( lightDef ) {
                  idEntity *temp = static_cast<idEntity *>(Mountedflashlight.GetEntity());
                  idAngles lightAng = firstPersonViewAxis.ToAngles();
                  idVec3 lightOrg = firstPersonViewOrigin;

                  idVec3 lightOffset = lightDef->GetVector( "light_offset" );
                  idVec3 lightAngleOffset = lightDef->GetVector( "light_angle_offset" );

                  gameLocal.SpawnEntityDef( *lightDef, &temp, false );
                  Mountedflashlight = static_cast<idLight *>(temp);

                  Mountedflashlight.GetEntity()->fl.networkSync = true;

                  lightOrg += (lightOffset.x * firstPersonViewAxis[0]);
                  lightOrg += (lightOffset.y * firstPersonViewAxis[1]);
                  lightOrg += (lightOffset.z * firstPersonViewAxis[2]);
                  lightAng.pitch += lightAngleOffset.x;
                  lightAng.yaw += lightAngleOffset.y;
                  lightAng.roll += lightAngleOffset.z;

                  Mountedflashlight.GetEntity()->GetPhysics()->SetOrigin( lightOrg );
                  Mountedflashlight.GetEntity()->GetPhysics()->SetAxis( lightAng.ToMat3() );

                  Mountedflashlight.GetEntity()->UpdateVisuals();
                  Mountedflashlight.GetEntity()->Present();
               }
            }
         break;
      }
...
void idPlayer::Interact( void ){
   if( !InteractTouchingTriggers() ){ //try to interact
   // add sound here
	   // add interaction hud event (pulse?)
...
void idPlayer::Think( void ) {
	...
	if ( objectiveSystemOpen || gameLocal.inCinematic || influenceActive || statsUIopen || startInvGui ) { // PHIL
	...
	// zooming
	if ( ( usercmd.buttons ^ oldCmd.buttons ) & BUTTON_ZOOM && !statsUIopen ) { // PHIL 
...
void idPlayer::Event_OpenPDA( void ) {
	if ( !gameLocal.isMultiplayer && !statsUIopen ) { // PHIL 
		TogglePDA();
	}
}
...
void idPlayer::ClientPredictionThink( void ) {
	...
	if ( objectiveSystemOpen || statsUIopen || startInvGui ) { // PHIL
		usercmd.forwardmove = 0;
		usercmd.rightmove = 0;
		usercmd.upmove = 0;
	}
...
// PHIL BEGIN
	msg.WriteBits( Mountedflashlight.GetSpawnId(), 32 );
// PHIL END
...
// PHIL BEGIN
	int lightSpawnId;
   lightSpawnId = msg.ReadBits( 32 );
   Mountedflashlight.SetSpawnId( lightSpawnId );
// PHIL END
...
void idPlayer::ToggleStatsDebug( void ) {

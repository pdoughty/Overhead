...
void idPlayerView::SingleView( idUserInterface *hud, const renderView_t *view ) {
	...
	// PHIL BEGIN
	if( player->startStatsGui){
      player->statsGui->Redraw( gameLocal.time);
      return;
   }
   if( player->startInvGui){
      player->invGui->Redraw( gameLocal.time);
      return;
   }
   if( player->startMapGui){
      player->mapGui->Redraw( gameLocal.time);
      return;
   }
// PHIL END
}
